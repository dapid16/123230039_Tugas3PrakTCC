...
import { PenLine, FileText, Check, AlertCircle } from "lucide-react";
...
const AddNote = ({ refresh }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Gagal menambah note", err);
      setError("Gagal menyimpan. Cek koneksi server.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <button
        ...
        className="
          ...
          text-[#0a0a0a] font-semibold text-[13px]
          rounded-xl py-3 px-5 w-full
